...
fun squareBetweenExists(m: Int, n: Int): Boolean {
    for (s in sqrt(m.toDouble()).toInt()..sqrt(m.toDouble()).toInt() + 1)
        if (sqr(s) in m..n) return true
    return false
}
...
fun squareSequenceDigit(n: Int): Int {
    ...
    return sqr / 10.0.pow(n1 - n.toDouble()).toInt() % 10
}
...
fun fibSequenceDigit(n: Int): Int {
    ...
    return fib(number) / 10.0.pow(n1 - n.toDouble()).toInt() % 10
}
